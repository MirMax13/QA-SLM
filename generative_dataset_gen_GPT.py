...
def main():
    ...
    for idx, image_b64 in enumerate(page_images):
        ...
        try:
            if request_count >= MAX_REQUESTS:
                print("🚫 Reached request limit. Saving and exiting.")
                break
            raw_primary = call_vision_chat_primary(image_b64, prev_text)
            increment_request_count()
            ...
            if request_count >= MAX_REQUESTS:
                print("🚫 Reached request limit. Saving and exiting.")
                break
            raw_secondary = call_vision_chat_secondary(image_b64, raw_primary)
            increment_request_count()
    ...
    # irrelevant_qas = generate_irrelevant_qas(n=100, batch_size=16)
    # increment_request_count()
